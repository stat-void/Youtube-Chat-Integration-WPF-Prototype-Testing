...
namespace Void.YoutubeAPI
{
    ...
    public static class YoutubeSaveData
    {
        ...
        private static void Initialize()
        {
            try
            {
                Trace.WriteLine("Youtube data path initialization started.");
                SetDirectory(AppContext.BaseDirectory);
            }
            catch (UnauthorizedAccessException e)
            {
                Trace.WriteLine($"Unable to create or load JSON data file - {e.Message}");
            ...
            Trace.WriteLine($"Save path set as {basePath + $"\\{_dataName}"}");
        }
